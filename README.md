...
- [Auxiliary search for lost persons](#auxiliary-search-for-lost-persons)
    ...
    - [一、项目摘要](#一项目摘要)
      ...
      - [2、市场机遇和前景](#2市场机遇和前景)
      - [3、产品的竞争优势](#3产品的竞争优势)
        - [（1）摄像设备的普及覆盖](#1摄像设备的普及覆盖)
        - [（2）系统的优势](#2系统的优势)
    - [二、国内外现状](#二国内外现状)
      - [1、百度寻人](#1百度寻人)
      - [2、中国儿童失踪预警平台](#2中国儿童失踪预警平台)
      - [3、新浪微博](#3新浪微博)
      - [4、众觅系统](#4众觅系统)
      - [5、商汤“智慧空间公共管理平台”](#5商汤智慧空间公共管理平台)
    - [三、技术分析](#三技术分析)
      - [1、系统架构介绍](#1系统架构介绍)
      - [2、深度学习与计算机视觉的发展现状](#2深度学习与计算机视觉的发展现状)
      - [3、Yolo v3算法简介](#3yolo-v3算法简介)
      - [4、SiamMask算法简介](#4siammask算法简介)
      - [5、产品技术实现原理](#5产品技术实现原理)
      - [6、产品技术优势分析](#6产品技术优势分析)
        - [（1）摄像设备的普及覆盖](#1摄像设备的普及覆盖-1)
        - [（2）yolo算法和siammask算法的优势](#2yolo算法和siammask算法的优势)
      - [7、产品技术专利说明](#7产品技术专利说明)
        - [（1）Yolo算法](#1yolo算法)
        - [（2）Siammask算法](#2siammask算法)
      - [8、产品技术研发趋势](#8产品技术研发趋势)
      - [9、Yolo v3相比与前两代的升级](#9yolo-v3相比与前两代的升级)
        - [（1） 多级预测](#1-多级预测)
        - [（2）loss不同](#2loss不同)
        - [（3）加深网络](#3加深网络)
        - [（4） Router](#4-router)
      - [10、对未来yolo版本的预测](#10对未来yolo版本的预测)
        - [（1） mAP 会继续提高：](#1-map-会继续提高)
        - [（2）实时检测会成为标配：](#2实时检测会成为标配)
        - [（3）模型小型化成为重要分支：](#3模型小型化成为重要分支)
...
![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/4.png?raw=true)　　
...
![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/5.png?raw=true)

Yolov3的网络结构如下图：　　

![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/6.png?raw=true)
...
![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/7.png?raw=true)

　　另外也可以直接去掉预测box的head，提高计算速度，包围框也可以通过mask计算得到。网络结构如下：　　

![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/8.png?raw=true)
...
![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/9.png?raw=true)
...
![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/10.png?raw=true)
...
![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/11.png?raw=true)　　
...
![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/12.png?raw=true)、

　　SiamMask相比之前的跟踪算法的state-of-the-art精度又改进不少。

　　下图是与现有的视频目标分割算法的精度的比较:

![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/13.png?raw=true)

　　虽然SiamMask精度并不是最高的，但速度却比其他算法快1到2个数量级！在真实应用中更有价值。
　　下图为mIoU-速度的散点图。

![](https://github.com/Vodkayu/Auxiliary-search-for-lost-persons/blob/main/images/14.png?raw=true)

#### 9、Yolo v3相比与前两代的升级　　

　　Yolov3相比与前两代识别更准确、泛华能力也更好。有了多个新的技术亮点：　　

##### （1） 多级预测　　

　　终于为 YOLO 增加了 top down 的多级预测，解决了 YOLO 颗粒度粗，对小目标无力的问题。v2 只有一个 detection，v3 一下变成了 3 个，分别是一个下采样的，feature map 为 13*13，还有 2 个上采样的 eltwise sum，feature map 为 26*26，52*52，也就是说 v3 的 416 版本已经用到了 52 的 feature map，而 v2 把多尺度考虑到训练的 data 采样上，最后也只是用到了 13 的 feature map，这应该是对小目标影响最大的地方。在论文中从单层预测五种 boundingbox 变成每层 3 种 boundongbox。　　
...
　　当预测的目标类别很复杂的时候，采用 logistic regression 进行分类是更有效的，比如在 Open Images Dataset 数据集进行分类。在这个数据集中，会有很多重叠的标签，比如女人、人，如果使用 softmax 则意味着每个候选框只对应着一个类别，但是实际上并不总是这样。复合标签的方法能对数据进行更好的建模。　　

##### （3）加深网络　　

　　采用简化的 residual block 取代了原来 1×1 和 3×3 的 block（其实就是加了一个 shortcut，也是网络加深必然所要采取的手段）。这和上一点是有关系的，v2 的 darknet-19 变成了 v3 的 darknet-53，为啥呢？就是需要上采样啊，卷积层的数量自然就多了，另外作者还是用了一连串的 3*3、1*1 卷积，3*3 的卷积增加 channel，而 1*1 的卷积在于压缩 3*3 卷积后的特征表示。　　
...
##### （1） mAP 会继续提高：
　　
　　随着模型训练越来越高效，神经网络层级的不断加深，信息抽象能力的不断提高，以及一些小的修修补补，未来的目标检测应用mAP会不断提升。　　

##### （2）实时检测会成为标配：　　

　　目前所谓的“实时”，工业界是不认可的。为什么呢，因为学术圈的人，验证模型都是建立在 TitanX 或者 Tesla 这类强大的独立显卡上，而实际的潜在应用场景中，例如无人机/扫地/服务机器人/视频监控等，是不会配备这些“重型装备”的。所以，在嵌入式设备中，如 FPGA，轻量级 CPU 上，能达到的实时，才是货真价实的。 　　

##### （3）模型小型化成为重要分支：　　

　　类似于 tiny YOLO 的模型分支会受到更多关注。模型的小型化是应用到嵌入式设备的重要前提。而物联网机器人无人机等领域还是以嵌入式设备为主的。模型剪枝/二值化/权值共享等手段会更广泛的使用。
